...
public class SetCid extends DamlRecord<SetCid> {
  ...
  public final UserRole.ContractId newCid;

  public SetCid(UserRole.ContractId newCid) {
    this.newCid = newCid;
  }
  ...
  public static ValueDecoder<SetCid> valueDecoder() throws IllegalArgumentException {
    return value$ -> {
      ...
      UserRole.ContractId newCid =
          new UserRole.ContractId(fields$.get(0).getValue().asContractId().orElseThrow(() -> new IllegalArgumentException("Expected newCid to be of type com.daml.ledger.javaapi.data.ContractId")).getValue());

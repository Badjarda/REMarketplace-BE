...
public class UserProfileKey extends DamlRecord<UserProfileKey> {
  ...
  public UserProfileKey(String operator, String user, Id id) {
   ...
   */
  @Deprecated
  public static UserProfileKey fromValue(Value value$) throws IllegalArgumentException {
    return valueDecoder().decode(value$);
  }

  public static ValueDecoder<UserProfileKey> valueDecoder() throws IllegalArgumentException {
    return value$ -> {
      ...
      return new UserProfileKey(operator, user, id);
    } ;
  }
  ...
  public static JsonLfDecoder<UserProfileKey> jsonDecoder() {
    return JsonLfDecoders.record(Arrays.asList("operator", "user", "id"), name -> {
          switch (name) {
        ...
        , (Object[] args) -> new UserProfileKey(JsonLfDecoders.cast(args[0]), JsonLfDecoders.cast(args[1]), JsonLfDecoders.cast(args[2])));
  }

  public static UserProfileKey fromJson(String json) throws JsonLfDecoder.Error {
  ...
  public boolean equals(Object object) {
    ...
    if (!(object instanceof UserProfileKey)) {
      return false;
    }
    UserProfileKey other = (UserProfileKey) object;
  ...
  @Override
  public int hashCode() {
    return Objects.hash(this.operator, this.user, this.id);
  }

  @Override
  public String toString() {
    return String.format("daml.marketplace.interface$.common.types.UserProfileKey(%s, %s, %s)", this.operator,
        this.user, this.id);
  }
}

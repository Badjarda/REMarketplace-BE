...
public class SetCid extends DamlRecord<SetCid> {
  ...
  public static JsonLfDecoder<SetCid> jsonDecoder() {
    return JsonLfDecoders.record(Arrays.asList("newCid"), name -> {
          switch (name) {
            case "newCid": return com.daml.ledger.javaapi.data.codegen.json.JsonLfDecoders.JavaArg.at(0, com.daml.ledger.javaapi.data.codegen.json.JsonLfDecoders.contractId(daml.daml.finance.interface$.account.account.Account.ContractId::new));
            default: return null;
          }
        }
  ...
  public String toString() {
    return String.format("daml.daml.finance.interface$.account.account.SetCid(%s)", this.newCid);
  }
}

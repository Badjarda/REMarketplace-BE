...
public class View extends DamlRecord<View> {
  ...
  public static JsonLfDecoder<View> jsonDecoder() {
    return JsonLfDecoders.record(Arrays.asList("token"), name -> {
          switch (name) {
  ...
  public JsonLfEncoder jsonEncoder() {
    return JsonLfEncoders.record(
        JsonLfEncoders.Field.of("token", apply(Token::jsonEncoder, token)));
  }
  ...
  public String toString() {
    return String.format("daml.daml.finance.interface$.instrument.token.instrument.View(%s)",
        this.token);
  }
}
